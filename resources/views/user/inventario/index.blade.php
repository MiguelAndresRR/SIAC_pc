...
<body>
    ...
    <div class="content-inventario">
        <div class="header">
            ...
            <a onclick="window.location.href='{{ route('user.reportes.inventario_pdf') }}'" class="pdfGenerar">
                <i class="fa fa-file-pdf"></i> Generar PDF
            </a>
        </div>
        @include('user.inventario.layoutinventario.filtros')
        <div id="tabla-inventario">
            @include('user.inventario.layoutinventario.tablainventario')
        </div>
    </div>
    <script src="{{ asset('js/Trabajador/inventario/verDetallesInventarioProductos.js') }}"></script>
